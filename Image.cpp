...
{
	AllocateMemory(m_Width * m_Height * BytesPerChannel(m_Format));
	if (data)
		SetData(data);
}
...
void Image::AllocateMemory(uint64_t size)
{
	...
	VkFormat vulkanFormat = ImageFormatToVulkanFormat(m_Format);
...
void Image::SetData(const void* data)
{
	...
	size_t upload_size = m_Width * m_Height * BytesPerChannel(m_Format);
